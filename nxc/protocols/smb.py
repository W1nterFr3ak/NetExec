...
"""
    ...
    def plaintext_login(self, domain, username, password):
        ...
        try:
            ...
            if self.args.get_tgt and not self.is_guest:
                ...
                tgt, cipher, _, session_key = getKerberosTGT(principal, self.password, domain, "", "", self.kdcHost)
                ccache = CCache()
    ...
    def hash_login(self, domain, username, ntlm_hash):
        ...
        try:
            ...
            if self.args.get_tgt and not self.is_guest:
                principal = Principal(self.username, type=constants.PrincipalNameType.NT_PRINCIPAL.value)
                tgt, cipher, _, session_key = getKerberosTGT(principal, "", domain, self.hash, "", self.kdcHost)
                ccache = CCache()
                ccache.fromTGT(ticket, sessionKey, sessionKey)
                ccache.saveFile(self.username + '.ccache')
